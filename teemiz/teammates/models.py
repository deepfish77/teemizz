...
class Member(models.Model):
   # name = models.ForeignKey(User, on_delete=models.CASCADE, related_name='member', blank=True)
    related_profession = models.OneToOneField(Profession, on_delete=models.CASCADE, blank=True)
...
class WhatWeNeed(models.Model):
    ...
    experience = models.CharField(max_length=100 , choices=EXPERIENCE, blank=True)

    
class TeamPositionsManager(models.Manager):
    def apply_to_position(self, request_user, position_to_add):
        position_ = TeamPositions.objects.get(position__name__iexact=position_to_add)
        user = request_user
        is_applied= False
...
class TeamPositions(models.Model):
    ...
    industry = models.ForeignKey(Industry, on_delete=models.CASCADE, blank=True)
    profession = models.ForeignKey(Profession, on_delete=models.CASCADE, blank=True)
    tags = models.ManyToManyField(Tag, blank=True)
    candidates = models.ManyToManyField(User, blank=True, related_name='profession_candidates')
    responsibilities = models.ManyToManyField(Responsibilities, blank=True)
    what_we_need = models.ManyToManyField(WhatWeNeed, blank=True)
    accepted_teammate = models.OneToOneField(User, blank=True, null=True, on_delete=models.CASCADE , related_name='accepted')
...
class Team(models.Model):
    ...
    type = models.CharField(max_length=20, choices=TEAM_TYPES, blank=True, default=None)
    name = models.CharField(max_length=100, blank=True)
    objective = models.CharField(max_length=100, blank=True)
    summary = models.TextField(blank=True, default=None)
    industry = models.ForeignKey(Industry, on_delete=models.CASCADE, blank=True, default=None)
    team_positions = models.ManyToManyField(TeamPositions, blank=True, default=None)
